...
const sportsPeople = [
  ...
  // Add more sports people here (up to 20)
];
...
const cardVariants = {
  ...
  const Articles = () => {
    return (
      <Box display="grid" gridTemplateColumns="repeat(auto-fill, minmax(300px, 1fr))" gap={6} p={5}>
        {sportsPeople.map((person, index) => (
          ...
          >
            ...
            >
              ...
              <Link href={person.url} color="teal.500" textAlign="center" display="block">
                Learn More
              </Link>
            </Box>
          </motion.div>
